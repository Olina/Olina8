...
class COlina implements ISingleton {
	...
	protected function __construct() {
		...
		// include the site specific config.php and create a ref to $olina to be used by config.php
		$olina = &$this;
  ...
  public function ThemeEngineRender() {
    ...
    $themePath 	= OLINA_INSTALL_PATH . '/' . $this->config['theme']['path'];
    ...
    if(isset($this->config['theme']['parent'])) {
      $parentPath = OLINA_INSTALL_PATH . '/' . $this->config['theme']['parent'];
      $parentUrl	= $this->request->base_url . $this->config['theme']['parent'];
    }
    
    // Add stylesheet name to the $olina->data array
    $this->data['stylesheet'] = $this->config['theme']['stylesheet'];
    
    // Make the theme urls available as part of $olina
  ...
  /**
   * Draw HTML for a menu defined in $olina->config['menus'].
   *
